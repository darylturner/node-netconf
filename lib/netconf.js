...
Client.prototype = {
    ...
    open: function(callback) {
        ...
        this.sshConn.on('ready', function establishSession() {
            self.sshConn.subsys('netconf', handleStream); //next function
            function handleStream(err, stream) {
                ...
                stream.on('data', function handleHello(chunk) {
                    buffer += chunk;
                    if (buffer.match(delim)) {
                        self.netconf.removeListener('data', handleHello);
                        parseHello(buffer); //next function
                    }
            ...
            function parseHello(buffer) {
                self.parse(buffer, function assignSession(err, message) {
